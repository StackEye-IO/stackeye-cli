...
func TestNewMaintenanceDeleteCmd(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
...
func TestMaintenanceDeleteCmd_YesFlag(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
...
func TestMaintenanceDeleteCmd_NoArgs(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
	cmd.SetArgs([]string{})
...
func TestMaintenanceDeleteCmd_TooManyArgs(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
	cmd.SetArgs([]string{
...
func TestMaintenanceDeleteCmd_InvalidUUID(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
	...
	expectedMsg := "invalid maintenance window ID"
	if err != nil && !strings.Contains(err.Error(), expectedMsg) {
		t.Errorf("Error = %q, want to contain %q", err.Error(), expectedMsg)
	}
}

func TestMaintenanceDeleteCmd_ValidUUID(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
	...
	// Should NOT fail with invalid maintenance window ID error
	if err != nil && strings.Contains(err.Error(), "invalid maintenance window ID") {
		t.Errorf("Should not get invalid maintenance window ID error for valid UUID: %v", err)
	}
}

func TestMaintenanceDeleteCmd_Aliases(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()

	// delete command doesn't define aliases (direct subcommand of maintenance)
	if len(cmd.Aliases) != 0 {
		t.Errorf("Expected no aliases for delete command, got %v", cmd.Aliases)
	}
}

func TestMaintenanceDeleteCmd_EmptyUUID(t *testing.T) {
	cmd := NewMaintenanceDeleteCmd()
