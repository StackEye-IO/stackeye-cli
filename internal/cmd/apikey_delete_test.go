...
func TestNewAPIKeyDeleteCmd(t *testing.T) {
...
func TestAPIKeyDeleteCmd_YesFlag(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
...
func TestAPIKeyDeleteCmd_NoArgs(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
	cmd.SetArgs([]string{})
...
func TestAPIKeyDeleteCmd_TooManyArgs(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
	cmd.SetArgs([]string{
...
func TestAPIKeyDeleteCmd_InvalidUUID(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
	...
	expectedMsg := "invalid API key ID"
	if err != nil && !strings.Contains(err.Error(), expectedMsg) {
		t.Errorf("Error = %q, want to contain %q", err.Error(), expectedMsg)
	}
}

func TestAPIKeyDeleteCmd_ValidUUID(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
	...
	if err != nil && strings.Contains(err.Error(), "invalid API key ID") {
		t.Errorf("Should not get invalid UUID error for valid UUID: %v", err)
	}
}

func TestAPIKeyDeleteCmd_Aliases(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()

	expectedAliases := []string{"rm", "remove", "revoke"}
...
func TestAPIKeyDeleteCmd_EmptyUUID(t *testing.T) {
	cmd := NewAPIKeyDeleteCmd()
