...
func TestNewStatusPageDomainVerifyCmd(t *testing.T) {
	cmd := NewStatusPageDomainVerifyCmd()

	if cmd.Use != "domain-verify <id>" {
		t.Errorf("expected Use='domain-verify <id>', got %q", cmd.Use)
	}
	...
	if cmd.Short != "Get DNS verification record for custom domain" {
		t.Errorf("expected Short='Get DNS verification record for custom domain', got %q", cmd.Short)
	}
}

func TestNewStatusPageDomainVerifyCmd_Args(t *testing.T) {
	cmd := NewStatusPageDomainVerifyCmd()
...
func TestNewStatusPageDomainVerifyCmd_Long(t *testing.T) {
	cmd := NewStatusPageDomainVerifyCmd()

	long := cmd.Long

	// Should contain field documentation
	fields := []string{"HOST", "VALUE"}
	...
	// Should contain DNS configuration steps
	if !strings.Contains(long, "DNS Configuration Steps") {
		t.Error("expected Long description to contain DNS configuration steps")
	}

	// Should mention TXT record
	if !strings.Contains(long, "TXT record") {
		t.Error("expected Long description to mention TXT record")
	}
}

func TestRunStatusPageDomainVerify_Validation(t *testing.T) {
	// Test that validation errors are returned for invalid inputs.
	// Since runStatusPageDomainVerify requires an API client, validation happens first
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Call runStatusPageDomainVerify with a background context.
			// It should fail on validation before needing API client.
			err := runStatusPageDomainVerify(context.Background(), tt.statusPageID)
...
func TestRunStatusPageDomainVerify_ValidID(t *testing.T) {
	// Test that a valid ID passes validation (will fail later on API client)
	validID := "123"
	err := runStatusPageDomainVerify(context.Background(), validID)
...
func TestRunStatusPageDomainVerify_LargeValidID(t *testing.T) {
	// Test that large valid IDs work (within uint64 range)
	largeID := "999999999"
	err := runStatusPageDomainVerify(context.Background(), largeID)
